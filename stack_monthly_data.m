...
for i = 1:3%:length(d)
    ...
    nc.Time =  ncdateread(filename, 'time');
    nc.Time = nc.Time(1);
    nc = rmfield( nc , 'time' )

    % Variables to remove
    nc = rmfield( nc , 'specific_humidity_at_2m_agl');
    nc = rmfield( nc , 'cloud_base_height');
    nc = rmfield( nc , 'water_evaporation_amount');
    nc = rmfield( nc , 'lwe_percolation_rate');
    nc = rmfield( nc , 'surface_temperature');
    nc = rmfield( nc , 'surface_downward_latent_heat_flux');
    nc = rmfield( nc , 'surface_net_thermal_radiation');
    nc = rmfield( nc , 'air_pressure_at_surface');
    nc = rmfield( nc , 'lwe_runoff_surface_rate');
    nc = rmfield( nc , 'surface_downward_sensible_heat_flux');
    nc = rmfield( nc , 'surface_upward_latent_heat_flux_due_to_sublimation');
    nc = rmfield( nc , 'cloud_area_fraction');
    nc = rmfield( nc , 'visibility_in_air');
        ...
        for k = 1:length(vars)
            switch  (string(vars(k)))
                ...
                otherwise
                    ...
                    NC.(string(vars(k)))(:,:,i) = nc.(string(vars(k)));
            end
        end
    end
end
